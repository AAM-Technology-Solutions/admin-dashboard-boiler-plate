...
export function toEthiopianDateTime(eurDate) {
  if (!eurDateIsConvertible(eurDate)) return false;
  const difference = eurDate.getTime() - new Date(Date.UTC(1971, 8, 12)).getTime();
  const fourYearsPassed = Math.floor(difference / fourYears);
  let remainingYears = Math.floor((difference - fourYearsPassed * fourYears) / oneYear);
  if (remainingYears === 4) {
    remainingYears = 3;
  }
  const remainingMonths = Math.floor(
    (difference - fourYearsPassed * fourYears - remainingYears * oneYear) / (30 * oneDay),
  );
...
function toEuropeanDate(ethDate) {
  const initialEuropean = new Date(
    new Date(Date.UTC(ethDate.year, ethDate.month - 1, ethDate.date)).getTime() +
...
function toEuropeanDateString(ethDate) {
  let EuropeanDate = toEuropeanDate(ethDate);
